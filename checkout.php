...
<div class="container">
 ...
 <th scope="col">Reserve ID</th>
  ...
  -->
        <div class="table-responsive-lg" style="height: 250px; width:175%; margin-left:20px;">
         ...
         //     if(isset($_POST['submitSearch'])){
                ...
                $q = "SELECT `p_id`, reserve.status, passenger.name, `NID`, reserve.reserve_id, rooms.room_id, `phone`, `address`, `email`, `birthdate`, `start`, `end`, rooms.floor, rooms.adult, reserve.payment, `quality`   FROM passenger, reserve, roomreserve, rooms WHERE reserve.status=0 AND passenger.p_id = reserve.passenger_id  AND reserve.reserve_id = roomreserve.reserve_id AND rooms.room_id  = roomreserve.room_id";
                $res = selectV3($q, "i");
                $i=1;
                if($res!="0"){
                  ...
                  //     foreach($result as $row)
                   ...
                   //      echo("8");
                         ...
                         if($row['status']==0){
                           ...
                           $status = "<button onclick='toggle_status($row[reserve_id],1,$row[room_id])' class='btn btn-warning btn-sm shadow-none'>check out</button>";
                         }
                         else{
              ...
              //      echo $row['payment'];
                //    $pa="2300000";
                    ...
                    $adult = "'".$row2['adult']."'";
                      ...
                      <td>
                          <button type='button' onclick=\"infoRo($adult, $floor)\" class='btn btn-info shadow-none btn-sm' data-bs-toggle='modal' data-bs-target='#infoRo'>
                          $row2[room_id]
                          </button>
                      </td>
                          <td>$row2[name]</td>
...
function infoRe(start, end,p){
  ...
  //   document.querySelector("#info .modal-title").innerText = Reserveid;
    ...
    function toggle_status(id, val, room)
    {
        console.log("%%%%%");
        let xhr = new XMLHttpRequest();
        xhr.open("POST", "ajax/checkout_ajax.php", true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
      ...
      //    let  x=this.responseText; 
        ...
        xhr.send('toggle_status='+id+'&value='+val+'&r='+room);
    }
