        ...
        <div class="container-fluid" style="margin-left:220px;">
            <div class="row">
                <div class="col-md-2 ps-0 mb-3">
                    ...
                    <input type="text" name="<?php echo $i.'oldId'; ?>" value="0" class="form-control shadow-none">
                </div>
            </div>
            <div class="row">
                <div class="col-md-2 ps-0 mb-3">
...
<button name="Infoback" type="submit"  class="btn custom-bg text-white" style="width:15%; margin-left:660px; margin-top:20px;">back</button>
</form>
...
<?php
   ...
   //     echo $_SESSION['roomId'];
        $q = "UPDATE `rooms` SET `status`='0' WHERE `room_id`='$_SESSION[roomId]'";
        $r =update($q, 'ii');
        if($r==1){

            $q = "delete from roomreserve order by roomreserve_no desc limit 1";
            $r =delete($q);
            if($r==1){

                $q = "delete from reserve order by reserve_id  desc limit 1";
                $r =delete($q);
                if($r==1){
 
                    $_SESSION["IDs"] =$_SESSION["pid"];
                //     $_SESSION["n"] =$frm_data['n'];
                //    echo "Session variables are set.";
                $n=$_SESSION['IDs'];
                    redirect("Home.php?Id=$n");
               
            //   header("Location:Home.php?Id=$n");
                }
                else{
                    echo 3;
                }
